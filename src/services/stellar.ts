...
async function submitToHorizon(tx: Transaction): Promise<string> {
  try {
    ...
    // Preserve TimeoutError and TransportError for the retry wrapper's
    // classifier — wrapping them in Errors.upstream would lose the type
    // information that distinguishes transient from permanent failures.
    if (e instanceof TimeoutError || e instanceof TransportError) {
      throw e;
    }
    const codes =
